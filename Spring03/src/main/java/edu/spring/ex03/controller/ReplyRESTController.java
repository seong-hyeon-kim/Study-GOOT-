...
public class ReplyRESTController {
	...
	public ResponseEntity<List<ReplyVO>> readReplies(
			...
			// @PathVariable("boardId") : /all/{baordId} 값을 설정된 변수에 저장
		logger.info("여긴 되냐 사발");
	...
	public ResponseEntity<Integer> updateReply(
		...
		return new ResponseEntity<Integer>(result, HttpStatus.OK);
	} // end ResponseEntity<Integer> updateReply()
	
	@DeleteMapping("/{replyId}")
	public ResponseEntity<Integer> deleteReply(@PathVariable("replyId") int replyId,
			@RequestBody ReplyVO vo) {
		logger.info("REST delete() 호출 : replyId = " + replyId);
		int result = 0;
		try {
